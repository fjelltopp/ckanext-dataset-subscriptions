...
def create_user_with_resources(with_activity):
    subscribed_user = factories.User(name='user1', activity_streams_email_notifications=True)
...
def test_send_notification(mock_request):
    user=create_user_with_resources(with_activity=True)
    notifications = email_notifications.get_notifications(user, datetime.datetime.fromtimestamp(3600))
    message_result = actions.latin_username_send_notification(user, notifications[0])
    assert message_result == None
